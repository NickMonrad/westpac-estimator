    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b dark:border-gray-700">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Import Templates CSV</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {step === 'upload' && (
            <div className="space-y-4">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Upload a CSV to create new templates or update existing ones by matching on <code className="bg-gray-100 dark:bg-gray-700 px-1 rounded">TemplateName</code>.
              </p>
              <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center">
                ...
                {csvText && <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{csvText.split('\n').length - 1} data rows</p>}
              </div>
              <a href="/api/templates/export-csv" className="text-xs text-gray-400 dark:text-gray-500 underline">Download current templates as CSV template</a>
          ...
          {step === 'review' && preview && (
            <div className="space-y-4">
              ...
              {preview.newTemplates.length > 0 && (
                <div>
                  <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">New templates ({preview.newTemplates.length})</p>
                  <div className="space-y-1">
                    {preview.newTemplates.map((t, i) => (
                      <div key={i} className="flex items-center gap-2 text-sm p-2 bg-green-50 dark:bg-green-950/30 rounded">
                        ...
                        {t.category && <span className="text-gray-400 dark:text-gray-500">· {t.category}</span>}
                        <span className="text-gray-400 dark:text-gray-500 ml-auto">{t.taskCount} tasks</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {preview.updatedTemplates.length > 0 && (
                <div>
                  <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Templates to update ({preview.updatedTemplates.length})</p>
                  <div className="space-y-2">
                    {preview.updatedTemplates.map((t, i) => (
                      <div key={i} className="p-2 bg-yellow-50 dark:bg-yellow-950/20 border border-yellow-200 dark:border-yellow-800 rounded">
              ...
              {preview.newTemplates.length === 0 && preview.updatedTemplates.length === 0 && preview.errors.length === 0 && (
                <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-4">No changes detected.</p>
              )}
          ...
          {step === 'done' && (
            <div className="text-center py-8">
              ...
              <p className="text-gray-700 dark:text-gray-300 font-medium">Import complete</p>
            </div>
          )}
        </div>

        {/* Footer */}
        <div className="flex justify-between items-center p-4 border-t dark:border-gray-700 gap-2">
          {step === 'upload' && (
            <>
          ...
          {step === 'review' && (
            <>
              <button onClick={() => setStep('upload')} className="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">← Back</button>
              <button
          ...
          {step === 'done' && (
            <button onClick={onClose} className="ml-auto bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm px-4 py-2 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600">
              Close
            </button>
          )}
        </div>
