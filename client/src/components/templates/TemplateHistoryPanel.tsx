...
export default function TemplateHistoryPanel({ templateId, templateName, onRestored }: Props) {
  ...
  return (
    <div className="border-t pt-4 mt-4">
      <h3 className="text-sm font-semibold text-gray-700 mb-3">History — {templateName}</h3>
      ...
      <div className="flex gap-2 mb-4">
        ...
        <button
          ...
          className="bg-gray-100 text-gray-700 text-xs px-3 py-1 rounded hover:bg-gray-200 disabled:opacity-50 whitespace-nowrap"
        >
      ...
      {isLoading ? (
        ...
        <div className="space-y-2 max-h-72 overflow-y-auto">
          {snapshots.map(snap => (
            <div key={snap.id} className="border border-gray-200 rounded-lg text-xs">
              <div className="flex items-center justify-between px-3 py-2 bg-gray-50 rounded-t-lg">
                <div>
                  <span className="font-medium text-gray-700">{snap.label ?? triggerLabel[snap.trigger] ?? snap.trigger}</span>
                  <span className="ml-2 text-gray-400">{new Date(snap.createdAt).toLocaleString()}</span>
                  <span className="ml-2 text-gray-400">· {snap.snapshot?.tasks?.length ?? 0} tasks</span>
                </div>
              ...
              {expanded === snap.id && (
                <div className="px-3 py-2 bg-white rounded-b-lg border-t border-gray-100">
                  {snap.snapshot?.tasks?.length === 0 ? (
                    <p className="text-gray-400 italic">No tasks in this snapshot</p>
                  ) : (
